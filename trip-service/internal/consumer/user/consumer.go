...
const (
	...
	tripCancelQueueName = "trip-cancel-req"
)
...
func (c *Consumer) ConsumeTripCancel(ctx context.Context) {
	q, err := c.ch.QueueDeclare(
		tripCancelQueueName,
		false,
		false,
	...
	go func() {
		for m := range msgs {
			...
			if err := c.tripServive.CancelTrip(context.Background(), userID); err != nil {
